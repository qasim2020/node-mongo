...
var sendCode = function(code,req) {
  return new Promise((resolve,reject) => {
    ...
    var textURL = `http://Lifetimesms.com/plain?username=famousfakir&password=${process.env.PASSWORD}&to=${req.user.phone}&from=letsHelp&message=${code}`
    console.log(textURL);
    // resolve(code);
    axios.get(textURL)
      .then((response) => {
...
var sendText = (text,phone) => {
  return new Promise((resolve,reject) => {
    ...
    var textURL = `http://Lifetimesms.com/plain?username=famousfakir&password=${process.env.PASSWORD}&to=${phone}&from=letsHelp&message=${text}`
    console.log(textURL);
    // resolve(true);
    axios.get(textURL)
      .then((response) => {
